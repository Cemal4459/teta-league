...
export default function HomePageClient({ initialUser, initialProfile }: { initialUser: any, initialProfile: any }) {
  ...
  return (
    <div style={{ paddingBottom: '40px', paddingLeft: '40px', paddingRight: '40px', maxWidth: '1800px', margin: '0 auto', display: 'grid', gridTemplateColumns: '280px 1fr 380px', gap: '24px', minHeight: 'calc(100vh - 120px)' }}>
      ...
      {/* =========================================
          SOL ŞERİT (SIDEBAR - DYNAMIC PROFILE)
          ========================================= */}
      <div className="client-glass" style={{ borderRadius: '16px', display: 'flex', flexDirection: 'column', padding: '32px 24px', height: 'calc(100vh - 140px)', position: 'sticky', top: '100px', border: '1px solid rgba(64,224,208,0.1)', boxShadow: '0 10px 30px rgba(0,0,0,0.5), inset 0 0 20px rgba(64,224,208,0.02)' }}>
        {initialUser ? (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1 }}>
            
            <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end', marginBottom: '-10px', position: 'relative', zIndex: 1 }}>
              <span style={{ fontSize: '0.7rem', color: 'var(--brand-main)', letterSpacing: '2px', fontWeight: 'bold' }}>{initialProfile?.platform || 'PS5'}</span>
            </div>

            <div style={{ width: '120px', height: '120px', borderRadius: '50%', background: 'rgba(0,0,0,0.5)', border: '2px solid rgba(64,224,208,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px', boxShadow: '0 0 30px rgba(64,224,208,0.15)', overflow: 'hidden' }}>
              {initialProfile?.avatar_url ? (
                <img src={initialProfile.avatar_url} alt="Avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                <span style={{ fontSize: '3rem', fontWeight: '900', color: 'rgba(255,255,255,0.2)' }}>?</span>
              )}
            </div>
            
            <h2 className="font-bold" style={{ fontSize: '1.4rem', margin: '0 0 4px 0', letterSpacing: '1px', textShadow: '0 0 15px rgba(64,224,208,0.5)', textAlign: 'center' }}>
              {initialProfile?.ea_id || initialProfile?.username || 'BİLİNMEYEN'}
            </h2>
            <div className="text-muted" style={{ fontSize: '0.85rem', marginBottom: '24px', letterSpacing: '1px' }}>
              Serbest Oyuncu
            </div>
            
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '16px', background: 'rgba(255,255,255,0.02)', padding: '16px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span className="text-muted" style={{ fontSize: '0.8rem', fontWeight: 'bold' }}>POZİSYON</span>
                <span style={{ color: '#FFB800', fontWeight: '900', textShadow: '0 0 10px rgba(255,184,0,0.5)' }}>{initialProfile?.main_position || '--'}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span className="text-muted" style={{ fontSize: '0.8rem', fontWeight: 'bold' }}>FORMA</span>
                <span className="font-bold" style={{ color: '#fff' }}>{initialProfile?.jersey_number || '--'}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span className="text-muted" style={{ fontSize: '0.8rem', fontWeight: 'bold' }}>REYTİNG</span>
                <span className="font-bold" style={{ color: 'var(--brand-main)', textShadow: '0 0 10px rgba(64,224,208,0.5)' }}>0.0</span>
              </div>
            </div>

            <Link href="/profile" className="flat-button interactive" style={{ width: '100%', marginTop: 'auto', textAlign: 'center', background: 'rgba(255,184,0,0.1)', border: '1px solid rgba(255,184,0,0.3)', color: '#FFB800', letterSpacing: '2px', fontWeight: 'bold' }}>
              OYUNCUM
            </Link>
          </div>
        ) : (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div className="client-glass" style={{ padding: '24px', borderRadius: '16px' }}>
          ...
          <Link 
            href="/league?tab=stats" 
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', width: '100%', marginTop: '20px', padding: '12px', fontSize: '0.85rem', color: 'var(--text-muted)', textDecoration: 'none', fontWeight: 'bold', transition: 'all 0.3s' }} 
            onMouseEnter={e => { e.currentTarget.style.color = 'var(--brand-main)'; e.currentTarget.style.textShadow = '0 0 10px rgba(64, 224, 208, 0.5)'; }} 
            onMouseLeave={e => { e.currentTarget.style.color = 'var(--text-muted)'; e.currentTarget.style.textShadow = 'none'; }}
          >
            TÜM İSTATİSTİKLERİ DETAYLI İNCELE <span style={{ fontSize: '1rem' }}>&rarr;</span>
          </Link>
        </div>
